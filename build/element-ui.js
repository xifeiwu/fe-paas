...
class PreBuilder {
  ...
  async readConfig(custom) {
    const commentReg = /^ *\/\/.*/;
    const content = (await promisifyReadFile(path.resolve(DIR_ELEMENT_OUTPUT, `${custom}.json`))).toString()
      .split('\n').filter(it => {
  ...
  async generateJs(custom) {
    ...
    const OUTPUT_PATH = path.resolve(DIR_ELEMENT_OUTPUT, `${custom}.js`);
...
`;
  ...
  async generateCss(custom) {
    ...
    themes.forEach(async (theme) => {
      ...
      // generate index scss
      fs.writeFileSync(path.resolve(THEME_DIR, 'src', isSCSS ? 'index.scss' : 'index.css'), indexContent);
      // build scss
